...
describe "BeermappingApi" do
  ...
  describe "in case of a cache miss" do
    ...
    it "when HTTP GET returns one entry, it is parsed and returned" do
      ...
      stub_request(:get, /.*turku/).to_return(body: canned_answer, headers: @xml_header)
  ...
  describe "in case of a cache hit" do
    ...
    it "when one entry found in cache, it is returned" do
  ...
  it "when HTTP GET doesn't return anything, an empty array is returned" do

    canned_answer = <<-END_OF_STRING
    <?xml version='1.0' encoding='utf-8' ?><bmp_locations><location><id></id><name></name><status></status><reviewlink></reviewlink><proxylink></proxylink><blogmap></blogmap><street></street><city></city><state></state><zip></zip><country></country><phone></phone><overall></overall><imagecount></imagecount></location></bmp_locations>
    END_OF_STRING

    stub_request(:get, /.*imaginaryland/).to_return(body: canned_answer, headers: @xml_header)

    places = BeermappingApi.places_in("imaginaryland")
  ...
  it "when HTTP GET returns multiple entries, all are parsed and returned" do
    ...
    stub_request(:get, /.*helsinki/).to_return(body: canned_answer, headers: @xml_header)

    places = BeermappingApi.places_in("helsinki")

    expect(places.count).to eq(12)
    place = places.first
